...
public class RegisterWindowController {
    ...
    private void registerUser() {
        ...
        if (!isValidEmail(email)) {
            JOptionPane.showMessageDialog(view, "Ingrese un email válido (nombre@email.dominio).", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        if (!success) {
            JOptionPane.showMessageDialog(view, "El nombre de usuario o email ya están registrados.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // Si todo es correcto, abrir MainWindow y cerrar RegisterWindow
        JOptionPane.showMessageDialog(view, "Registro exitoso. Bienvenido a Skillify!", "Éxito", JOptionPane.INFORMATION_MESSAGE);
        new MainWindow();
        view.dispose();
    }
